...
namespace TestEmail
{
    ...
    {
        // downloaded from http://www.dimastr.com/redemption/download.htm
            ...
            lock (_criticalSection)
            {
                if (_redemptionDllHandle.Equals(IntPtr.Zero))
                {
                    ...
                    if (_dllGetClassObjectPtr.Equals(IntPtr.Zero))
                        throw new Exception(
                            "Could not retrieve a pointer to the 'DllGetClassObject' function exported by the dll");
                ...
                Object unk;
                var hr = _dllGetClassObject(ref guid, ref IID_IClassFactory, out unk);
